...
Capability-token systems such as UCAN and CACAO solve adjacent but different problems: proving delegated authority at signing time. This proposal focuses instead on resolver-native current-state authorization, allowing relying parties to verify, in real time, whether a credential remains authorized, revoked, expired, or rotated under an ENS name. The architectures are complementary and can compose together at the relying-party layer.
...
**Positioning vs. adjacent layers.** The toolkit layers above naming and discovery (ENSIP-25/26/64) and registry (ERC-8004/8122); complements wire-protocol authentication layers (MCP, A2A); serves as the ENS-native authority-resolution layer for delegated execution systems (ERC-4337); composes with capability-token presentation (UCAN, CACAO; see [Appendix B](./appendices/appendix-b-capability-tokens.md)) at the relying-party layer.

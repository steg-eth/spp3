...
`mouz.eth`'s specialism is precisely this proposal's highest-risk surface: the Verifier and per-name AuthResolver, whose entire job is to make an authorization decision that holds up against an adversary, hardened by a funded third-party audit (§6/M6).

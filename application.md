...
**The strategic shape.** ENS grows by absorbing new payloads issued under operator-managed subnames — the cb.id precedent (Coinbase's branded subname issuance, absorbing millions of ENS-bound users) is the proof point. MARPs are the next operator class, and the toolkit ships both the payload format (AuthResolver records) and the verification runtime (Verifier contract) they need to issue agent-identity subnames at scale — letting MARPs (or any third party) build managed agent identity platforms (MAIPs) on top of ENS. The protocol engineering is the service — the substrate operator classes adopt to grow ENS itself ([full case in Steg's ENS forum post, May 2026](https://discuss.ens.domains/t/the-next-operator-class-managed-agent-runtime-platforms/22121)).

**Vendor-lock alternative.** Without a shared MAIP substrate, each MARP ships its own vertically integrated identity-and-authorization stack — each becomes the platform, the gatekeeper, and the off-switch for the agents it hosts. Microsoft Entra Agent ID is the leading enterprise prototype of this closed pattern; the open-source agent ecosystem inherits it by default unless an alternative ships first. Steg ships that alternative — an ENS authority lookup layer any MARP can adopt or fork. A counterparty resolving an agent's authority lookup gets the same answer regardless of which MARP issued the credential, with no MARP-internal platform layer, no centralized gatekeeper, and no off-switch outside the operator's own ENS name.
...
It is the next ENS payload primitive for the next operator class — extending ENS by the same growth pattern (cb.id, Coinbase) that absorbed millions of users into ENS-bound identity.
